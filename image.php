...
<body>
    <header class="haut">
        <p class="haut_gauche"><font size="8px">Importation d'image</font></p>
        <p class="haut_droit1">
    ...
    <main>
        <div class="principal">
            ...
            <form method="post" action="" enctype="multipart/form-data">
                <label for="file">Image (png. jpeg, jpg, gif)</label><br/><br/>
                <input type="file" name="imag" id="file"/><br/><br/><br/>
...
<?php
    ...
    if(isset($_FILES['imag']) AND $_FILES['imag']['error']==0){
        if($_FILES['imag']['size']<=1000000000){
            $infosfichier=pathinfo($_FILES['imag']['name']);
            $extention_upload=$infosfichier['extension'];
            $exten_auto=array('png','jpeg','jpg','gif','img');
            if(in_array($extention_upload,$exten_auto)){
                move_uploaded_file($_FILES['imag']['tmp_name'],'images/'.basename($_FILES['imag']['name']));
                $taille = $_FILES['imag']['size']/1024;
                $req->execute(array(
                    'ut' => $_GET['personne'],
                    'im' => $_FILES['imag']['name'],
                    'mu' => '',
